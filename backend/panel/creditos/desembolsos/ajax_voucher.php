...
switch($_GET['consulta']){
    case 'buscar': $query = "
                            SELECT iddesembolso, idcliente, us_ases.idusuario as idases, us_adm.idusuario as idadm, us_caja.idusuario as idcaja, idcredito, idsolicitud, conyugue.idconyugue AS idconyugue, aval.idconyugue AS idaval, clientes.nombre AS cli_nombre, clientes.apellido_pat AS cli_apellido_pat, clientes.apellido_mat AS cli_apellido_mat, fecha_desem,
    ...
    case 'editar': $id = $_GET['id'];
                   $query = "
                            SELECT iddesembolso, idcliente, us_ases.idusuario as idases, us_adm.idusuario as idadm, us_caja.idusuario as idcaja, idcredito, idsolicitud, conyugue.idconyugue AS idconyugue, aval.idconyugue AS idaval, desembolso.fecha_desem,
...
while($row = $result->fetch_array()){
    ...
    $json[] = array(
        'fecha' => $fecha,
        'fecha_desem' => date("d-m-Y H:i:s",strtotime($row['fecha_desem'])),
